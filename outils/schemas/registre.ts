 ...
 * schéma »). Les vingt-deux schémas combinent `if`/`then`/`else`, `not` et `contains` à travers des
 * branches que l'heuristique statique de ajv ne peut pas suivre ; en `strict: true`, le premier
 * appel de validation lève une exception avant d'atteindre le premier des 123 exemples. Les
...
/**
 * Charge les vingt-deux schémas dans un unique registre ajv (draft 2020-12 + ajv-formats). C'est la seule
